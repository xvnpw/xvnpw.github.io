...
What makes deep analysis special is its multi-document output strategy. Depending on what you're analyzing, the tool generates different sets of documentation:
...
## Warning on using Deep Analysis

While Deep Analysis Mode provides richer insights, it's essential to be mindful of a few points:

- **Verification Challenges**: The in-depth outputs can be even harder to verify than those from Normal Mode. Always exercise caution and critically assess the results generated by Large Language Models (LLMs).
- **Model Limitations**: New generation AIs like Gemini 2.0 are better at creating detailed outputs, but they are not without halucinations. Be aware of potential inaccuracies.
...
What is important to note, is that deep analysis mode has first steps same as in normal mode. We need to have high level overview of the project before we can go deeper.
...
Comparing the two modes yielded insightful observations:

- **Depth vs. Brevity**: Deep Analysis Mode offers a thorough exploration of threats, which can be valuable for in-depth security planning. However, it may include verbose content that doesn't necessarily enhance practical understanding.
- **Verification Challenges**: The detailed outputs from Deep Analysis Mode can be more challenging to verify and validate.
- **AI Capabilities**: Models like Gemini 2.0 Flash have improved in generating detailed outputs but still require careful scrutiny.

## `sec-docs` Repository

I generated documentation using new Deep Analysis Mode and stored it in the [sec-docs](https://github.com/xvnpw/sec-docs) repository. You can find it by looking at per language folder or by looking at specific project:

{{< figure src="https://github.com/user-attachments/assets/3c85e401-c11e-4034-bfff-b97306748e63" class="image-center" >}}

## Looking Forward

The dual-mode approach of the AI Security Analyzer shows promise in scaling security analysis while serving different needs:

- **Normal Mode**: Ideal for quick overviews and starting points for further investigation.
- **Deep Analysis Mode**: Suited for comprehensive reviews where detailed insights are necessary.
