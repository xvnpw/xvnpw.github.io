...
Some of you asked for more detailed analysis of projects in `sec-docs` repository. I didn't know if Gemini 2.0 is able to provide more deep dives, so I decided to try it out. Having in mind that Google will soon remove free access to Gemini 2.0 Flash, I rushed to implement it. Final results are not perfect, but I think in many cases it can be useful.
...
- **`github` Mode Only**: Deep Analysis Mode is currently available only in `github` mode. I'm not sure if it will be available in other modes. I need to test it with other models. Currently, it's much tailored to Gemini 2.0 Flash.
...
- **Model Limitations**: New generation AIs like Gemini 2.0 are better at creating detailed outputs, but they are not without hallucinations. Be aware of potential inaccuracies.
...
You can check other prompts for deep analysis in the [prompts.py](https://github.com/xvnpw/ai-security-analyzer/blob/337cd9aa56e9ae66017f04fe3fdbb3c2f855a21e/ai_security_analyzer/prompts.py#L844C1-L850C4) and how they are used in the [github2tm_agents.py](https://github.com/xvnpw/ai-security-analyzer/blob/337cd9aa56e9ae66017f04fe3fdbb3c2f855a21e/ai_security_analyzer/github2tm_agents.py#L165).
...
I generated new set of documentation for 1000 projects from my [sec-docs](https://github.com/xvnpw/sec-docs) repository. You can find it by looking at per language folder or by looking at specific project:
